...
export async function GET(req: NextRequest) {
  try {
    ...
    const actualPumpState = searchParams.get('actual_pump_state') || searchParams.get('esp_pump_state');
    ...
    if (actualPumpState) {
      updateData.espPumpState = actualPumpState.toLowerCase();
    }
    if (ackVersion !== null && !isNaN(ackVersion)) {
      updateData.ackConfigVersion = ackVersion;
    }

    const state = await prisma.fertigationControl.upsert({
      where: { id: 1 },
      update: updateData,
      create: {
        id: 1,
        ...
        espPumpState: actualPumpState ? actualPumpState.toLowerCase() : 'off',
        lastHeartbeat: now,
        lastUpdated: now,
      },
    });
